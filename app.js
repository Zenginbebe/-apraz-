...
function createMacColumn(name, mac) {
  ...
  return `
    <div class="column">
      ...
      <div class="accordion shown-block">
...
function simulateChanges() {
  document.querySelectorAll("td.odd").forEach(td => {
    ...
    if (newVal > prev) indicator.classList.add("up");
    if (newVal < prev) indicator.classList.add("down");
    indicator.innerHTML = newVal > prev ? "↑" : newVal < prev ? "↓" : "";
...
function initAkordeon() {
  document.querySelectorAll(".match-title").forEach(title => {
    title.addEventListener("click", () => {
      // aynı sıradaki tüm .accordion divleri eş zamanlı toggle
      const matchTitle = title.textContent.trim();
      document.querySelectorAll(".column").forEach(column => {
        const otherTitle = column.querySelector(".match-title").textContent.trim();
        if (otherTitle === matchTitle) {
          const accordion = column.querySelector(".accordion");
          accordion.classList.toggle("shown-block");
        }
      });
    });
  });
